...
public class LmForm {
    ...
    public static void Form0(Player player) {
        ...
        form.addElement(new ElementInput(Form0_Element29, ProtectWorld.toString().replaceAll("\\[", "").replace("]", ""), ProtectWorld.toString().replaceAll("\\[", "").replace("]", "")));
        form.addElement(new ElementToggle(Form0_Element23, RealTime));
        form.addHandler(FormResponseHandler.withoutPlayer((ignored) -> {
            if (!form.wasClosed()) {
                ...
                String TaskDelay = form.getResponse().getInputResponse(23);
                boolean DoubleJump = form.getResponse().getToggleResponse(24);
                ArrayList<String> ProtectWorld = new ArrayList<>(Arrays.asList(form.getResponse().getInputResponse(25).split(", ")));
                boolean RealTime = form.getResponse().getToggleResponse(26);
                ...
                LmConfig.ProtectWorld = ProtectWorld;
                LmConfig.RealTime = RealTime;
                saveConfig();
                loadLangConfig();
